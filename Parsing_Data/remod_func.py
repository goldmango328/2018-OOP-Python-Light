...
def jaju_PhantomJS(path = '/Users/지명금/Desktop/phantomjs-2.1.1-windows/phantomjs-2.1.1-windows/bin/phantomjs'):
    # PhantomJS_path를 다운로드 받아야 함!
    PhantomJS_path = path
    driver = webdriver.PhantomJS(PhantomJS_path)
    driver.implicitly_wait(1)

def remod_chair(room_size, pic_chart):
    url = 'http://www.remod.co.kr/product/list.html?cate_no='
    CHAIR = furni(room_size,url+'139&page=1',chart) # 하나당 5개씩만
    ...
    ch_url, ch_name, ch_price, ch_img, ch_size = CHAIR.print_all()
    return ch_name, ch_price, ch_size, ch_img

def remod_closet(room_size, pic_chart):
    url = 'http://www.remod.co.kr/product/list.html?cate_no='
    DRESSER = furni(room_size,url+'175&page=1',chart)
    dr_url, dr_name, dr_price, dr_img, dr_size = DRESSER.print_all()
    print dr_name, dr_price, dr_size, dr_img

def remod_sofa(room_size, pic_chart):
    url = 'http://www.remod.co.kr/product/list.html?cate_no='
    SOFA = furni(room_size,url+'138&page=1',chart)
    SOFA.changeURL(url + '138&page=2')
    SOFA.changeURL(url + '138&page=3')
    sf_url, sf_name, sf_price, sf_img, sf_size = SOFA.print_all()
    return sf_name, sf_price, sf_size, sf_img

def remod_table(room_size, pic_chart):
    url = 'http://www.remod.co.kr/product/list.html?cate_no='
    TABLE = furni(room_size, url+'134&page=1',chart)
...
if __name__ == "__main__":
    ...
    CHAIR = furni(url+'139&page=1',chart) # 하나당 5개씩만
    CHAIR.changeURL(url+'139$page=2')
    CHAIR.changeURL(url+'139&page=3')
    for i in range(1,4):
        CHAIR.changeURL(url+'140%page='+str(i))
